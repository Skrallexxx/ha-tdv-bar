    ...
    if(entity.d!=0&&/*this._tracker.bar_id!=baridx*/this._tracker.hist_offset==null&&entity.ap!=null)
     {
      ...
      if(entity.allow_neg) zeroposbaroffset=Math.round((width-bar_x-1)/2); else zeroposbaroffset=0;
        ...
        for(let s=gridstep;s<entity.max_raw;s+=gridstep)
         {
          ...
          if(a<(dp-2))
           { 
            ...
            else if(entity.d<0&&entity.allow_neg)
             {
...
//console.log(">>>>>>>>>>>>>>>>>>>>>>>>>",this.fonts.name,this.metric.nameheight)
    ...
    if(this._tracker.data!=null&&this._tracker.data!=0&&this._tracker.bar_id!=null&&this._tracker.bar_id==baridx&&(this.trackingmode==1||this.trackingmode==3))
     {
      ...
      else if(this._tracker.data<0&&entity.allow_neg)
       { 
    ...
    if(entity.allow_neg)// Draw zero line
     {
        ...
        if(entity.h[i]&&entity.h[i].v)
         {
          ...
          else if(entity.allow_neg) this.ctx.lineTo(chart_x+i+1,(y+(height-zeroposchartoffset)+a));
         }
       }
      this.ctx.stroke();
